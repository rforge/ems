...
#' \dontrun{
#' set.seed(1)
#' data <- data.frame(sex = sample(c("M","F"), 50, rep = T), age = sample(c(NA,20:70), 50, rep = T), admissionType = sample(c(NA,"urgency", "clinical", "scheduled"), 50, rep = T), hospitalizationTime = sample(c(0:10), 50, rep = T), numberOfChildren = sample(c(NA,0:3), 50, rep = T), cancerInFamily = sample(c(NA,T,F), 50, rep = T), diabetesInFamily = sample(c(T,F), 50, rep = T), thrombosisInFamily = sample(c(T,F), 50, rep = T), mentaldiseasesInFamily = sample(c(T,F), 50, rep = T), cardiadicdiseaseInFamily = sample(c(NA,T,F), 50, rep = T), readmission = sample(c(NA,T,F), 50, rep = T))
#' 
#' attach(data)
...
#' tableStack(cancerInFamily:cardiadicdiseaseInFamily, data, by= readmission) # "by" compares variables
#' tableStack(cancerInFamily:cardiadicdiseaseInFamily, data, by= readmission, prevalence=TRUE) # "prevalence" returns the prevalence instead of the absolute values
#' tableStack(cancerInFamily:cardiadicdiseaseInFamily, data, by= readmission, percent=FALSE) # "percent" as FALSE hides the percentage in parenthesis
#' tableStack(cancerInFamily:cardiadicdiseaseInFamily, data, by= readmission, percent=FALSE, name.test=FALSE) # "name.test" as FALSE hides the column that shows the tests names
#' tableStack(cancerInFamily:cardiadicdiseaseInFamily, data, by= readmission, NAcol = TRUE) # "NAcol" displays a column of NA values on the variable on "by"
#' tableStack(cancerInFamily:cardiadicdiseaseInFamily, data, by= readmission, NAcol = TRUE, NArow = TRUE) # "NArow" displays rows of NA values on the variables on "vars"
#' 
#' 
#' tableStack(vars=2:7, data, by=sex) # the specification of the vars may be done as the range
#' tableStack(vars=2:7, data, by="none") # "by" var may be specified as "none" and the selected vars will be crossed only against the total
#' tableStack(vars=2:7, data, by = NONE) # by = NONE works just as by = "none"
#' tableStack(vars=2:7, data, by=sex, total.column=TRUE) # total.column displays a column of totals in adition to the variable on by
#' 
#' var.labels <- c("sex", "Type of admission for each patient", "age", "Duration time in days of the patient's hospitalization", "Number of children that the patient have", "whether or not the patient has cancer in family", "whether or not the patient has diabetes in family", "whether or not the patient has thrombosis in family", "whether or not the patient has mental diseases in family", "whether or not the patient has cardiac diseases in family", "whether or not the patient is on a relapse admission")
#' attr(data, "var.labels") <- var.labels #setting the attribute var.labels
#' rm(var.labels)
#' 
#' tableStack(vars=c(numberOfChildren,hospitalizationTime), data)  # May need full screen of Rconsole
#' tableStack(vars=c(numberOfChildren,hospitalizationTime), data, var.labels.trunc=35) # Fits in with default R console screen
#' tableStack(vars=c(age,numberOfChildren,hospitalizationTime), data, reverse=TRUE) -> a
#' a
...
#' 
#' tableStack(c(age, numberOfChildren,hospitalizationTime,mean.score,total.score), data, by=sex, test=FALSE)  # hiding the test column
#' tableStack(3:5, data, by=sex, iqr=hospitalizationTime) # variables specified on iqr will not display SD but IQR instead
#' tableStack(3:5, data, by=admissionType, iqr=c(hospitalizationTime, total.score))
#' ## 'vars' can be mixture of variables of different classes
#' data$highscore <- mean.score > 4
#' tableStack(mean.score:highscore, data, by=sex, iqr=total.score) # a variable with some comparison may be created easily
#' 
#' tableStack(vars=c(readmission,admissionType), data, by=sex, percent="none") # the percentage information may be hidden
#' tableStack(vars=c(readmission,admissionType), data, by=sex, prevalence = TRUE) # it may be shown the prevalende of the variable instead of the values themselves
#' tableStack(vars=c(readmission,admissionType), data, by=sex, name.test = FALSE) # the name of the tests may be hidden while the test itself still shows
#' 
#' ## Variable in numeric or factor
#' tableStack(vars=3:5, data, by=sex) # as continuous varaibles
#' tableStack(vars=3:5, data, by=sex, vars.to.factor = 3:5) # as factors
#' 
#' ## Using drplvls
#' # a dataframe will be created containing a factor with an unused level
#' bloodbank <- data.frame(AgeInDays =
#'     sample(0:15,200, replace = T), Type =
#'     factor(sample(c("A","B","0"), 200, replace = T),
#'       levels = c("A","B","AB","0")), Origin =
#'     sample(c("US","CA"), 200, replace = T))
#' 
#' # by using drplvls the row of the unused fator is hidden
#' tableStack(vars = c(AgeInDays, Type), bloodbank, by = Origin) #usual
#' tableStack(vars = c(AgeInDays, Type), bloodbank, by = Origin, drplvls = TRUE) # with drplvls
#' 
