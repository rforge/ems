...
#'
#' @param y.type A character vector representing the indicator type. It is used to name the vertical axis if \code{option = "rate"}) or \code{option = "ratioRate"s}), ignored otherwise. Usually is 'SMR' or 'SRU'.
#'
...
#'
#' @param theta Target value which specifies the desired expectation for institutions considered "in control". Used when \code{option = "prop"} or \code{option = "rate"}.
#'
...
#'
#' @param myunits A character vector with the unit names which one would like to benchmark among all units. Default is \code{NULL}.
#'
...
#'
#' @param option A character specifying the type of funnel plot one wants to produce. It can assume \code{"rate"}, \code{"ratioRates"}, \code{"prop"}, \code{"diffProp"} or \code{"ratioProp"}. See details to understand how to use each of them.
#'
#' @param printUnits Logical; If \code{TRUE} (default), the units are identified in the plot and printed in de console. The numbers plotted correspond to the table row names printed in the console.
#'
#' @param plot Logical; If \code{TRUE} (dafault), the correspondent graphic is plotted.
#'
...
#'
#' @param col Specification character vector for the CI lines colors. Must have same length of \code{p} + 1 with the target line color in the last position.
#'
#' @param lwd A value specifying the lines width, a positive number. It's the same for all lines in the plot. See \code{\link[graphics]{par}}.
#'
